...
```mermaid
flowchart TB
    A["에이전트 호출"] --> C["그 스레드의 저장된 메시지를 목록에 채운다"]
    C --> D["관찰된 메시지를 목록에서 빼고<br/>관찰 기록을 system 자리에 넣는다"]
    D --> E["모델에게 보낸다"]
    E --> S[("mastra_messages<br/>새 메시지를 저장한다")]
    S --> C
    A --> P{"버퍼링 간격에 닿았나"}
    P -- 예 --> G["비동기로 Observer 를 부른다<br/>응답을 막지 않는다"]
    G --> H[("mastra_observational_memory<br/>관찰 기록과 관찰한 메시지 id 목록")]
    H -. "어느 메시지를 관찰했는지" .-> D
    D --> T{"기준 토큰에 닿았나"}
    T -- "예, 버퍼 있음" --> I["즉시 적용한다. 멈추지 않는다"]
    T -- "예, 버퍼 없음" --> J["동기로 Observer 를 부른다<br/>대화가 그동안 멈춘다"]
    I --> H
    J --> H
```
